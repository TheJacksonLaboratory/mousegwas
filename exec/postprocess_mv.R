...
if (args$coat_phenotype){
  for (p in phenos){
    ...
    pnames <- rbind(pnames, data.frame(Group="NoGroup", PaperName=gsub(" ", "", gsub("(coat|eyes)", "", p))))
  }
}
...
ggsave(filename = paste0(args$plotdir, "/combined_figure1.svg"),
       ...
       units = "in")
if (args$meanvariance){
  combp <- plot_grid(pvep, meanplot, varplot, mainplot, nrow = 4, ncol = 1, labels = c('A', 'B', 'C', 'D'), label_size = 12, fontface="plain", rel_heights = c(2, 1, 1, 1))
